...
namespace _7Wonders
{
    class DropDown : TextPlusVisual
    {
        protected Dictionary<string,TextPlusVisual> strings;
        ...
        public DropDown(Vector2 _pos, int _w, int _h, List<string> _options, bool? _enabled = true)
            : base(_pos, _w, _h, _options[0], "Font1","grayback", Color.White, Color.Gray)
        {
            enabled = (_enabled.HasValue) ? _enabled.Value : true;
            strings = new Dictionary<string, TextPlusVisual>();
            int count=1;
            foreach (string s in _options)
            {
                strings.Add(s, (new TextPlusVisual(position + new Vector2(0, count * height), width + 21, height, s, "Font1", "grayback", Color.White, Color.Orange)).setVisible(false));
                count++;
            }
        ...
        public override void Update(GameTime gameTime, MouseState mState)
        {
            ...
            foreach (TextPlusVisual v in strings.Values)
            {
                if (v.isClicked())
                {
                    text = v.getText();
