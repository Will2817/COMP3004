    ...
    public class Side
    {
        ...
        public Side(JObject _json)
        {
            ...
            foreach (JObject stage in (JArray)_json["stages"])
            {
                ...
                foreach (JObject j in (JArray)stage["effects"])
                {
        ...
        public Dictionary<Resource, int> getStageCost(int stageNum)
        {
            return stageCosts[stageNum];
        }

        // Returns a List of Effects for that particular stage
        public List<Effect> getStageEffects(int stageNum)
        {
            return stageEffects[stageNum];
        }
